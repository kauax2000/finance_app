    ...
    >
      ...
      {Array.from({ length: thumbCount }, (_, index) => (
        <SliderPrimitive.Thumb
          ...
          key={index}
          className="block size-4 shrink-0 rounded-full border border-primary/50 bg-background shadow-xs ring-ring/70 transition-colors hover:ring-4 focus-visible:ring-4 focus-visible:outline-hidden disabled:pointer-events-none disabled:opacity-50"
        />
      ))}
    </SliderPrimitive.Root>
  )
