-- Ensure subscription billing copies payment fields into transactions.
--
-- Context: An earlier migration defined `run_subscription_billing()` inserting
-- `payment_method` / `payment_credit_card_id` as NULL. Later migrations replaced
-- it to copy these fields from `workspace_subscriptions`, but environments with
-- partial migration history can end up with the stale behavior.
--
-- This migration redefines the function to always copy payment fields.
